...
const Technology = ({ technologyPromise }: ITechnologyPromiseType) => {
  ...
  const handleOnClick = (technology: ITechnologyType) => {
    const validation = addTechnology.find(
  ...
  const handleRemoveingSingleTecnology = (tech: number | string) => {
    const removeTechnology = addTechnology.filter(
    ...
    <section className=" container mx-auto  ">
      ...
      <div className=" grid grid-cols-12 container mx-auto gap-5 items-start">
        <div className=" col-span-9 ">
          <div className=" grid  grid-cols-3 gap-5">
            {technologyData.map((technology) => {
              const isSelected = addTechnology.some(
                (item) => item.id === technology.id,
              );
              return (
                <div
                  key={technology.id}
                  className="technology-card border-2 shadow-xl border-[#F1F5F9] space-y-2 p-5 rounded-md"
                >
                  <div className="flex justify-between items-center ">
                    ...
                    <div
                      className={`${technology.bgColor} ${technology.color} `}
                    >
                      {technology.badge}
                    </div>
                  </div>
                  ...
                  <p className="text-sm text-[#64748B] line-clamp-3 min-h-15  ">
                  ...
                  <button
                    ...
                    className={`text-sm py-2 w-full rounded-xl flex items-center justify-center gap-2     border  cursor-pointer ${
                      isSelected
                        ? "bg-white text-[#D82C20] border-[#D82C20] font-semibold"
        ...
        <div className=" p-5 border-2 shadow-sm border-[#F1F5F9] rounded-2xl col-span-3  ">
          ...
          <p className="text-[#94A3B8] text-sm mt-2.5 mb-4">
            {addTechnology.length > 0 ? addTechnology.length : "No"}{" "}
          ...
          {addTechnology.length === 0 ? (
            ...
            <div className="space-y-3">
              {addTechnology.map((tech) => (
                <div
                  key={tech.id}
                  className="flex items-center justify-between p-3 bg-[#F1F5F9] rounded-lg"
                >
                  <div>
                    <img
                      src={tech.icon}
                  ...
                  <div
                    onClick={() => handleRemoveingSingleTecnology(tech.id)}
